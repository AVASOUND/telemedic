...
import { queryAppointmentsForDoctor } from "@/mypolybase/polybase";
...
export default function DoctorAppointmentList(props) {
    ...
    useEffect(()=>{
        async function getAppointments(){
            const results = await  queryAppointmentsForDoctor(props.doctor.id)
            console.log(results)
            setAppointments(results)
            props.stats([
              { name: "Number of appointments", value: results.length },
              { name: "Average time per appointment", value: 1, unit: "hours" },
              { name: "Number of clients seen", value: results.length },
